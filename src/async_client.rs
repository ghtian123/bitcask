...
impl AsyncKvsClient {
    ...
    async fn send_request(&mut self, req: request) -> Result<Option<response>, KvError> {
        ...
        self.stream.send(Bytes::from(serde_json::to_vec(&req)?)).await?;
       
        if let Some(Ok(data)) = self.stream.next().await {

            let res_reader = serde_json::Deserializer::from_slice(data.as_ref()).into_iter::<response>();
    ...
    pub async fn set(&mut self, key: String, value: String) ->  Result<(), KvError> {
        self.send_request(request::Set { key: key, value: value }).await.unwrap();
        Ok(())
    }

    pub async fn remove(&mut self, key: String) -> Result<(), KvError> {
        self.send_request(request::Remove { key: key }).await.unwrap();
